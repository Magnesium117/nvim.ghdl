local M = {}
local newdecoder=require("ghdl.luajson")
...
----------------------------------------------------------------------------------------
function M.ToggleTerminal(command, direction, closeonexit, title)
...
M.jsondecode=newdecoder()
M.insert=function(fs,fn)
  local found=false
  for _,value in ipairs(fs.files) do
    if(value.file==fn) then
      found=true
      break
    end
  end
  if(not found) then
    table.insert(fs.files,{["file"]=fn,["language"]="vhdl"})
  end
  return fs
end
